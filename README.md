...
Structured knowledge with revision provenance, permissions, and citation-grounded answers.

AtlasBase is a focused, public MIT distribution for the `knowledge` module in [managed-oss-cloud](https://github.com/rohanarun/managed-oss-cloud). It includes a product web UI, a product-scoped HTTP client, the `atlasbase` CLI, and a stdio MCP server exposing only this product's 9 typed actions.

![AtlasBase sample workspace](./docs/product-workspace.png)
...
npm install
npm link
export ATLASBASE_TOKEN="a-scoped-workspace-token"
export ATLASBASE_URL="https://cloud.getsupers.com"
atlasbase actions
atlasbase workspace
atlasbase page '{"limit":50,"state":"active","search":"Title prefix"}'
atlasbase detail '00000000-0000-4000-8000-000000000001'
atlasbase action library-create '{"name":"Support handbook","defaultAccess":"workspace","locale":"en-US","reviewCadenceDays":90,"idempotencyKey":"knowledge.library-create.sample-0001"}'
```
...
export ATLASBASE_TOKEN="a-scoped-workspace-token"
export ATLASBASE_URL="https://cloud.getsupers.com"
export ATLASBASE_WEB_KEY="a-separate-random-browser-key"
npm start
```
...
```bash
docker build -t atlasbase:0.3.0 .
...
```json
{
  "mcpServers": {
    "atlasbase": {
      "command": "atlasbase-mcp",
      "env": {
        "ATLASBASE_TOKEN": "a-scoped-workspace-token",
        "ATLASBASE_URL": "https://cloud.getsupers.com"
      }
    }
  }
}
...
Then point `ATLASBASE_URL` at the self-hosted control-plane origin. All products may share the same backend and PostgreSQL cluster while the backend preserves workspace and module boundaries.

## Typed action catalogue

| Action ID | Capability | Token scope | Operation |
|---|---|---|---|
| `library-create` | Create governed library | `write` | `command` |
| `page-revision-draft` | Draft immutable page revision | `write` | `command` |
| `page-revision-publish` | Publish exact page revision | `write` | `command` |
| `source-link` | Link verifiable source | `write` | `command` |
| `answer-propose` | Propose cited knowledge answer | `ai` | `ai` |
| `staleness-audit-propose` | Propose staleness audit | `ai` | `ai` |
| `permission-grant` | Grant scoped library access | `write` | `command` |
| `page-export` | Export exact page revision | `write` | `command` |
| `import-preview` | Preview knowledge import | `read` | `read` |
